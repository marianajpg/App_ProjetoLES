...
const ConsultaPedidos = () => {
  ...
  const [pedidos, setPedidos] = useState([
    ...
    { idCompra: '77889', idProduto: '5', cliente: 'Pedro Martins', total: 150.0, status: 'processamento' },
  ]);

  const handleStatusChange = (idCompra, novoStatus) => {
    setPedidos(pedidos.map(pedido => 
      pedido.idCompra === idCompra ? { ...pedido, status: novoStatus } : pedido
    ));
  };
  ...
  const pedidosFiltrados = pedidos.filter(pedido => {
    ...
    const correspondePesquisa = 
      pedido.idCompra.toLowerCase().includes(termo) ||
      pedido.cliente.toLowerCase().includes(termo);

    const correspondeAba = abaAtiva === 'geral' || status === abaAtiva;

    const correspondeFiltro = 
      valoresFiltro.status.length === 0 || valoresFiltro.status.includes(status);
  ...
  const renderStatusButton = (pedido) => {
    switch (pedido.status) {
      case 'processamento':
        return <button className="status-button status-em_transito" onClick={() => handleStatusChange(pedido.idCompra, 'em_transito')}>Em Trânsito</button>;
      case 'em_transito':
        return <button className="status-button status-entregue" onClick={() => handleStatusChange(pedido.idCompra, 'entregue')}>Confirmar Entrega</button>;
      case 'entregue':
        return <button className="status-button status-troca" onClick={() => handleStatusChange(pedido.idCompra, 'troca')}>Troca</button>;
      case 'troca':
        return <button className="status-button status-troca" disabled>Troca Solicitada</button>;
      default:
        return null;
    }
  }
